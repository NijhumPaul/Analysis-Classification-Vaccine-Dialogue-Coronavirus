...
def social_features(vaccine_data):
    ...
    for index, row in vaccine_data.iterrows():
        ...
        if is_quote == True:
            ...
            is_quotes = 0
        is_quote_list.append(is_quotes)
        is_tweet = row['is_retweet']
        if is_tweet == 'TRUE':
            is_tweets = 1
        else:
            is_tweets = 0
        is_tweet_list.append(is_tweets)
        ...
        if verified ==  'TRUE':
            ...
            verifieds = 0
        verifieds_list.append(verifieds)'''
...
'''def word_embedding(preprocessed_data):
    ...
    model = gensim.models.Word2Vec(tokens, size=100, min_count=1, workers=4)
    ...
    for index, row in preprocessed_data.iterrows():
        ...
        for word in nl.word_tokenize(sentence):
            ...
            first_values.append(word_2_vec_value[0])
            second_values.append(word_2_vec_value[1])
        #word_array = [statistics.mean(first_values), statistics.mean(second_values)]
        if len(first_values)>0:
            first_value_means.append(statistics.mean(first_values))
        else:
            first_value_means.append(0.0)
        if len(second_values)>0:
            second_value_means.append(statistics.mean(second_values))
        else:
    ...
    print("\nPrediction word2vec : \n", predict)
    print("Score word2vec : \n", sklearn.metrics.accuracy_score(Y, predict))
